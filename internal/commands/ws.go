...
func handleWs(args string, ctx *Context) error {
	...
	default:
		return ctx.Sender.SendMarkdown(ctx.ChatID,
			i18n.T("Usage: `/ws [list|save <name>|use <name>|remove <name>]`"), ctx.MessageID)
	}
}
...
func wsSave(name string, ctx *Context) error {
	if name == "" {
		return ctx.Sender.SendMarkdown(ctx.ChatID, i18n.T("Usage: `/ws save <name>`"), ctx.MessageID)
	}
	cwd := ctx.Workspaces.CwdFor(ctx.Scope, ctx.Config.Workspace.Default)
	if cwd == "" {
		return ctx.Sender.SendMarkdown(ctx.ChatID,
			i18n.T("No working directory set. Use `/cd <path>` first, then `/ws save <name>`."), ctx.MessageID)
	}
	if err := ctx.Workspaces.SaveNamed(name, cwd); err != nil {
		return err
	}
	return ctx.Sender.SendMarkdown(ctx.ChatID,
		fmt.Sprintf(i18n.T("Saved workspace alias `%s` -> `%s`."), name, cwd), ctx.MessageID)
}

func wsUse(name string, ctx *Context) error {
	if name == "" {
		return ctx.Sender.SendMarkdown(ctx.ChatID, i18n.T("Usage: `/ws use <name>`"), ctx.MessageID)
	}
	cwd := ctx.Workspaces.GetNamed(name)
	if cwd == "" {
		return ctx.Sender.SendMarkdown(ctx.ChatID,
			fmt.Sprintf(i18n.T("No workspace alias named `%s`. Use `/ws` to list."), name), ctx.MessageID)
	}
	resolved, err := workspace.Resolve(cwd)
	if err != nil {
		return ctx.Sender.SendMarkdown(ctx.ChatID,
			fmt.Sprintf(i18n.T("Alias `%s` points to `%s` which is no longer valid: %s"), name, cwd, err),
			ctx.MessageID)
	}
	...
	return ctx.Sender.SendMarkdown(ctx.ChatID,
		fmt.Sprintf(i18n.T("Switched to `%s` (`%s`). Session reset."), name, resolved), ctx.MessageID)
}

func wsRemove(name string, ctx *Context) error {
	if name == "" {
		return ctx.Sender.SendMarkdown(ctx.ChatID, i18n.T("Usage: `/ws remove <name>`"), ctx.MessageID)
	}
	if !ctx.Workspaces.RemoveNamed(name) {
		return ctx.Sender.SendMarkdown(ctx.ChatID,
			fmt.Sprintf(i18n.T("No workspace alias named `%s`."), name), ctx.MessageID)
	}
	return ctx.Sender.SendMarkdown(ctx.ChatID,
		fmt.Sprintf(i18n.T("Removed workspace alias `%s`."), name), ctx.MessageID)
}
