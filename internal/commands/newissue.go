...
5. 创建前先查重：用 gh issue list --search 或 gh search issues 在目标仓库搜索相似 issue（含已关闭的）。若发现高度相似的 issue，不要重复创建，改为回复该 issue 链接并简述与本次讨论的对应关系；若只是部分相关，在新 issue 正文中引用它。
6. 正文写两个版本：先完整英文版，再加一个中文版（用 "## 中文版本" 分节），两版内容一致。中文版必须保留模板原有的标题、字段名和结构（不翻译模板本身），只把填写的内容翻译成中文。
7. 讨论记录中标注了本地图片路径的，先查看图片内容再撰写。
8. 目标仓库唯一确定后无需向我确认，直接用 gh issue create 创建。
9. 创建成功后，回复 issue 链接和一句话摘要；失败时说明原因，不要重试超过一次。
{{extra}}
