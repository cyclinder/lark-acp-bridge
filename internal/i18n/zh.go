...
var zh = map[string]string{
	...
	// --- card: help ---------------------------------------------------------
	"Help": "帮助",
	"**Bridge commands** (always available, no token cost):":                "**桥接命令**（始终可用，不消耗 token）：",
	"**%s commands** (no token cost):":                                      "**%s 命令**（不消耗 token）：",
	...
	"`/resume` — list and resume past sessions":                             "`/resume`（/恢复）— 列出并恢复历史会话",
