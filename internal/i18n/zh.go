...
var zh = map[string]string{
	...
	// --- commands: help -----------------------------------------------------
	"/model — list available models (current marked); /model N|name switches": "/model — 列出可用模型（标记当前）；/model 序号|名称 切换",
	"/resume — list saved sessions; /resume N to reconnect one":               "/resume — 列出已保存会话；/resume 序号 重新连接",
	"/sessions — list provider sessions; /sessions N to continue one":         "/sessions — 列出提供方会话；/sessions 序号 继续某个会话",
	...
	"/new /reset — clear the current chat session":                      "/new /reset — 清除当前聊天会话",
	"/cd <path> — switch working directory (resets session)":            "/cd <路径> — 切换工作目录（重置会话）",
	"/ws — manage named workspace aliases (/ws save|use|remove <name>)": "/ws — 管理工作区别名（/ws save|use|remove <名称>）",
	"/open [path] — create/reuse a group bound to a cwd (p2p only)":     "/open [路径] — 创建/复用绑定到工作目录的群（仅私聊）",
	"/status — show current state":                                      "/status — 查看当前状态",
	"/pwd — print the current working directory":                        "/pwd — 显示当前工作目录",
	"/stop — stop the active run":                                       "/stop — 停止当前运行",
	"/new-issue <repo> [--last N] [--since today|24h|7d] [extra prompt] — summarize recent group messages into a GitHub issue": "/new-issue <仓库> [--last N] [--since today|24h|7d] [附加提示词] — 将近期群消息总结成 GitHub issue",
	"Collect the latest 500 group messages, auto-summarize and file an issue (default): /new-issue spidernet-io/spiderpool":    "收集最近 500 条群消息，自动总结并提交 issue（默认）：/new-issue spidernet-io/spiderpool",
	"Collect only the last 100 messages: /new-issue spidernet-io/spiderpool --last 100":                                        "只收集最近 100 条消息：/new-issue spidernet-io/spiderpool --last 100",
	"Extra free text guides the summary: /new-issue spidernet-io/spiderpool focus on the RDMA discussion":                      "附加自定义提示词引导总结：/new-issue spidernet-io/spiderpool 重点看 RDMA 的讨论",
	...
	"/provider — list providers; /provider <id> to switch, /provider default to reset": "/provider — 列出提供方；/provider <id> 切换，/provider default 重置",
	"/help — this help":                                                                "/help — 显示本帮助",
